    ...
    <div className="min-h-screen bg-gray-50 pb-28 font-sans">
      ...
      <div className="px-4 mb-2">
        <div className="relative">
          <input
            ...
            className="w-full rounded-full border px-4 py-2 pl-10 text-sm text-black bg-white shadow"
            value={searchQuery}
      ...
      <main className="grid grid-cols-2 md:grid-cols-3 gap-4 px-4 mt-4">
        {filteredItems.map((item) => (
          <div key={item.id} className="bg-white rounded-xl shadow-sm hover:shadow-md transition-all">
            <Image
              src={item.image}
              alt={item.name}
              width={300}
            ...
            <div className="p-3">
              ...
              <button
                className="mt-2 w-full bg-blue-600 text-white text-xs py-1.5 rounded hover:bg-blue-700 transition-all"
        ...
        <footer className="fixed bottom-0 left-0 right-0 bg-white border-t px-6 py-4 shadow-lg z-20">
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-700 font-medium">
              {totalItems} Items | Rp {totalPrice.toLocaleString('id-ID')}
            </span>
            <Link href="/cart">
